...
float average_classe(int n, Student **tab, int i, char *classe)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            printf("Student average %s in the class %s: %f\n", tab[i]->name, tab[i]->classe, tab[i]->average);
        }
    }
    return *classe;
}
